...
if(user!= null){
	...
	BillingCycleDao billDao = new BillingCycleDao(DBCon.getConnection());
		bills = billDao.notPaidBillbyUser(user.getId());
...
<div class="container">
		<div class="card-header my-3 text-center ">All Pending Bills</div>
		<table class="table table-light ">
			...
			%>
				<tr>
					<td class="text-center" name="id"><%=bill.getBillId() %></td>
					<td class="text-center" name="userid"><%=bill.getId() %></td>
					<td class="text-center"><%=bill.getName() %></td>
					<td class="text-center"><%=bill.getUnit() %></td>
					<td class="text-center"><%=bill.getDate() %></td>
					<td class="text-center" name="bill"><%=bill.getAmount() %></td>
					<td class="text-center"><%=bill.getDuedate() %></td>
					<td class="text-center"><%=bill.getStatus() %></td>
					
					<td class="text-center"><a class="btn btn-sm btn-primary" href="paybill.jsp?id=<%=bill.getBillId()%>&userid=<%=bill.getId() %>&
					bill=<%=bill.getAmount() %>>">
					Pay</a></td>
				</tr>
				<%
